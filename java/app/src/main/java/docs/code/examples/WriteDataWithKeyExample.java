...
public class WriteDataWithKeyExample {
  ...
  public static void writeHRecordDataWithKey(HStreamClient client, String streamName) {
    ...
    for (int i = 0; i < 500; i++) {
      ...
      Record record;
      if ((i % 3) == 0) {
        HRecord hRecord = HRecord.newBuilder().put("id", i).put("temperature", temp).put("withKey", 1).build();
        record = Record.newBuilder().hRecord(hRecord).partitionKey(key1).build();
      } else {
        HRecord hRecord = HRecord.newBuilder().put("id", i).put("temperature", temp).put("withKey", 2).build();
    ...
    System.out.println("Wrote message IDs: " + recordIds.stream().map(CompletableFuture::join).collect(Collectors.toList()));
    producer.close();
  }
}
